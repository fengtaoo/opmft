...
def train(args, model, loss_fn, m_optim, m_scheduler, metric, train_loader, dev_loader, dev_loader_2, device, writer):
    ...
    for epoch in range(args.epoch):
        ...
        for step, train_batch in enumerate(train_loader):
            ...
            if (step+1) % 100 == 0:
                print(step+1, avg_loss/100, best_mes)
                writer.add_scalar("loss", avg_loss/100, step+1)
                avg_loss = 0.0
            ...
            if (step+1) % args.eval_every == 0:
                ...
                with torch.no_grad():
                    ...
                    writer.add_scalar("dev", mes, step+1)
                    writer.add_scalar("test", mes2, step+1)
                    print("Step " + str(step+1) + ": " + str(args.metric) + " = " + str(mes))
                if mes >= best_mes:
                    best_mes = mes
                print('save_model...')
                if torch.cuda.device_count() > 1:
...
def main():
    ...
    if args.reinfoselect:
        ...
        train(args, model, loss_fn, m_optim, m_scheduler, metric, train_loader, dev_loader, dev_loader_2, device, writer)
    writer.close()
