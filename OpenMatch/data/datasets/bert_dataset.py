    ...
    def collate(self, batch: Dict[str, Any]):
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                segment_ids_neg = torch.tensor([item['segment_ids_neg'] for item in batch])
                input_mask_neg = torch.tensor([item['input_mask_neg'] for item in batch])
                input_ids_pos_target = torch.tensor([item['input_ids_pos_target'] for item in batch])
                segment_ids_pos_target = torch.tensor([item['segment_ids_pos_target'] for item in batch])
                input_mask_pos_target = torch.tensor([item['input_mask_pos_target'] for item in batch])
                input_ids_neg_target = torch.tensor([item['input_ids_neg_target'] for item in batch])
                segment_ids_neg_target = torch.tensor([item['segment_ids_neg_target'] for item in batch])
                input_mask_neg_target = torch.tensor([item['input_mask_neg_target'] for item in batch])
                return {'input_ids_pos': input_ids_pos, 'segment_ids_pos': segment_ids_pos, 'input_mask_pos': input_mask_pos,
                        'input_ids_neg': input_ids_neg, 'segment_ids_neg': segment_ids_neg, 'input_mask_neg': input_mask_neg,
                        'input_ids_pos_target': input_ids_pos_target, 'segment_ids_pos_target': segment_ids_pos_target, 'input_mask_pos_target': input_mask_pos_target,
                        'input_ids_neg_target': input_ids_neg_target, 'segment_ids_neg_target': segment_ids_neg_target, 'input_mask_neg_target': input_mask_neg_target}
            elif self._task == 'classification':
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                inputs = example["input"]  # list
                query_tokens = self._tokenizer.tokenize(" [SEP] ".join(inputs))
                if len(query_tokens) > self._query_max_len:
                    # truncate at beginning
                    query_tokens = query_tokens[len(query_tokens) - self._query_max_len:]
                query_tokens_target = self._tokenizer.tokenize(example['query'])[:32]
                doc_tokens_pos = self._tokenizer.tokenize(example['doc_pos'])[:self._seq_max_len-len(query_tokens)-3]
                doc_tokens_neg = self._tokenizer.tokenize(example['doc_neg'])[:self._seq_max_len-len(query_tokens)-3]
                doc_tokens_pos_target = self._tokenizer.tokenize(example['doc_pos'])[:self._seq_max_len-len(query_tokens_target)-3]
                doc_tokens_neg_target = self._tokenizer.tokenize(example['doc_neg'])[:self._seq_max_len-len(query_tokens_target)-3]
                input_ids_pos, input_mask_pos, segment_ids_pos = self.pack_bert_features(query_tokens, doc_tokens_pos)
                input_ids_neg, input_mask_neg, segment_ids_neg = self.pack_bert_features(query_tokens, doc_tokens_neg)
                input_ids_pos_target, input_mask_pos_target, segment_ids_pos_target = self.pack_bert_features(query_tokens_target, doc_tokens_pos_target)
                input_ids_neg_target, input_mask_neg_target, segment_ids_neg_target = self.pack_bert_features(query_tokens_target, doc_tokens_neg_target)
                return {'input_ids_pos': input_ids_pos, 'segment_ids_pos': segment_ids_pos, 'input_mask_pos': input_mask_pos,
                        'input_ids_neg': input_ids_neg, 'segment_ids_neg': segment_ids_neg, 'input_mask_neg': input_mask_neg,
                        'input_ids_pos_target': input_ids_pos_target, 'segment_ids_pos_target': segment_ids_pos_target, 'input_mask_pos_target': input_mask_pos_target,
                        'input_ids_neg_target': input_ids_neg_target, 'segment_ids_neg_target': segment_ids_neg_target, 'input_mask_neg_target': input_mask_neg_target}
            ...
            if self._task == "new":
                ...
                query_tokens = self._tokenizer.tokenize(" [SEP] ".join(inputs))[:self._query_max_len]
                if len(query_tokens) > self._query_max_len:
                    # truncate at beginning
                    query_tokens = query_tokens[len(query_tokens) - self._query_max_len:]
                doc_tokens = self._tokenizer.tokenize(example['doc'])[:self._seq_max_len-len(query_tokens)-3]
